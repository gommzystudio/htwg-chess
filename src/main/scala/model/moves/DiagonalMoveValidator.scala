...
// DiagonalMoveValidator is responsible for determining valid diagonal moves for pieces like Bishops.
final case class DiagonalMoveValidator(dist: Int = 8) extends MoveValidator {

  // Calculates valid diagonal moves for a piece based on its current position and the state of the field.
  override def getValidMoves(
    ...
    // Four diagonal directions: top-right, bottom-right, bottom-left, and top-left.
    val directions = List((1, 1), (1, -1), (-1, 1), (-1, -1))

    // Collects new moves in all diagonal directions.
  ...
  // Generates moves in a specific diagonal direction up to a certain distance.
  private def generateMovesInDirection(
      piece: Piece,
      position: PositionInterface,
      field: FieldInterface,
      dx: Int, // X direction (-1 for left, 1 for right)
      dy: Int, // Y direction (-1 for down, 1 for up)
